...
namespace PathOfExile.Model
{
    internal class ItemConverter : JsonConverter
    {
            ...
            JsonSerializer serializer)
        {
            ...
            if (obj.category?.gems != null) return obj.ToObject<Gem>();
            if (obj.category?.currency != null) return obj.ToObject<Currency>();
            if (obj.category?.jewels != null) return obj.ToObject<Jewel>();
            if (obj.category?.cards != null) return obj.ToObject<Card>();
            if (obj.category?.flasks != null) return obj.ToObject<Flask>();
            if (obj.category?.maps != null) return obj.ToObject<Map>();
            if (obj.category?.leaguestones != null) return obj.ToObject<Leaguestone>();

            var jobj = (JObject) obj.category;
            if (jobj["monsters"] != null) return obj.ToObject<Beast>();
